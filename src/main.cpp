...
void loop()
{
  ...
  */
  ...
  if (millis() - SendStamp > SendDelay)
  {
    ...
    // Check device status.
    if ((int)SensorsCurrentValues.device_state == 0 || (int)SensorsCurrentValues.valveData[0].mix_valve_type == 0 || (int)SensorsCurrentValues.valveData[1].mix_valve_type == 0)
    {
